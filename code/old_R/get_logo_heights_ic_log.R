...
get_logo_heights_ic_log <- function(table, alpha = 1, epsilon = 0.01, bg = NULL, opt = 1,
  ...
  if (is.vector(bg)==TRUE){
    ...
    rownames(bgmat) <- rownames(table)
    colnames(bgmat) <- colnames(table)
    ...
    for(j in 1:dim(table_mat_neg_norm)[2]){
      if(sum(table_mat_neg_norm[,j]) == 0){
        table_mat_neg_norm[,j] <- normalize_ic_log(table_mat_neg_norm[,j]+1e-3)
      }
    }

    for(j in 1:dim(table_mat_pos_norm)[2]){
      if(sum(table_mat_pos_norm[,j]) == 0){
        table_mat_pos_norm[,j] <- normalize_ic_log(table_mat_pos_norm[,j]+1e-3)
      }
    }
