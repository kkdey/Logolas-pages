...
#' probability weighed KL splitting of ratios
#'
...
#' colnames(m) = 1:12
#' m=m/8
#' get_logo_heights_ic_probKL(m, alpha = 1, hist = FALSE)
#'
#' @export



get_logo_heights_ic_probKL <- function(table, alpha = 1, epsilon = 0.01, bg = NULL, opt = 1,
                                    hist = FALSE, quant = 0.5, symm = TRUE){
  ...
  table <- apply(table+0.0001,2,normalize_ic_probKL)
  bgmat <- apply(bgmat+0.0001,2,normalize_ic_probKL)
  ...
  for(j in 1:dim(table_mat_neg_norm)[2]){
    if(sum(table_mat_neg_norm[,j]) == 0){
      table_mat_neg_norm[,j] <- normalize_ic_probKL(table_mat_neg_norm[,j]+1e-3)
    }
  }

  for(j in 1:dim(table_mat_pos_norm)[2]){
    if(sum(table_mat_pos_norm[,j]) == 0){
      table_mat_pos_norm[,j] <- normalize_ic_probKL(table_mat_pos_norm[,j]+1e-3)
    }
  }
...
normalize_ic_probKL = function(x){return(x/sum(x[!is.na(x)]))}
